...
class Anggota extends CI_Controller{
    ...
    function index($offset=0,$order_column='nis',$order_type='asc'){
      ...
      public function export_excel(){    
        //$data['anggota']=$this->m_anggota->anggota()->result();
        $data['title']="Data Anggota";
        $data = array( 'title' => 'Laporan Excel',
        'anggota' => $this->m_anggota->listing());
        $this->load->view('anggota/laporan_excel',$data);
    }
    ...
    function profile($id){
        $data['petugas']=$this->m_anggota->petugas($id)->row();//result ditangkap jika ada foreach 

        $this->template->display('anggota/petugas',$data);
    }
