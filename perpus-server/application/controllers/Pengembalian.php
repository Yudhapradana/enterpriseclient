...
class Pengembalian extends CI_Controller{
    ...
    function simpan(){
        ...
        $this->m_pengembalian->hapusPeminjaman($no);
        $this->m_pengembalian->simpan($info);

        $txt=$this->m_pengembalian->cariBuku($no_transaksi)->row_array();// cari kode_buku
        
        $kode_buku= $txt['kode_buku']; 
        $stok= $txt['stok']+1; 
        
        $this->m_pengembalian->kembali_stok($kode_buku,$stok);


         // $buku=$this->m_pengembalian->cariBuku($no_transaksi)->result();// mencari kode bukunya
        // $id_buku=$buku['kode_buku'];// dapat kode bukunya
        // $stok=array(
        //     'stok'=>$buku['stok']+1);
        // print_r($stok);
        // die();
    ...
    function byId($id){
        $data['id']=$id;
        $this->template->display("pengembalian/pengembalian_by_id",$data);
    }
}
